...
# --- STM ADDED IMPORTS (Using langchain_classic as requested) ---
...
# --- MemoryManager Class to handle summarization logic ---
class MemoryManager:
    """
    Manages the conversational memory summary based on ConversationSummaryBufferMemory, 
    writing the result back to the LangGraph state's chat_history field.
    """
    def __init__(self, llm, max_token_limit=1000):
        self.llm = llm
        self.max_token_limit = max_token_limit 
        
    def update_summary(self, state: Dict[str, Any]) -> str:
        """
        Calculates the new summary string based on the current state's messages.
        """
        messages = state.get("messages", [])
        current_summary = state.get("chat_history", "")

        # Need at least two messages (user input + bot output) to summarize a turn
        if len(messages) < 2:
            return current_summary
        
        # 1. Get the history and summary *before* the current turn
        history_before_turn = messages[:-2]
        # The current exchange
        input_message = messages[-2].content 
        output_message = messages[-1].content 
        
        # 2. Create a temporary memory instance with the old history + old summary
        ...
        temp_memory.chat_memory.messages = history_before_turn
        temp_memory.moving_summary_buffer = current_summary
        
        # 3. Save the new context (the latest turn) to trigger pruning/summarization
        temp_memory.save_context(
            {"input": input_message},
            {"output": output_message}
        )
        
        # 4. The new summary is in the buffer
        new_summary = temp_memory.moving_summary_buffer
        
        print(f"[MEMORY] Summary updated. New length: {len(new_summary)}")
        return new_summary
# ---------------------------------------------
...
class GraphBuilder:
    def __init__(self):
        ...
        self.memory_manager = MemoryManager(self.llm, max_token_limit=1000) 
...
Choose the RIGHT tool based on the query type."""
    ...
    def _chatbot_node(self, state: State):
        """Enhanced chatbot node with caching, smart routing, and memory integration"""
        messages = state["messages"]
        ...
        if last_message and hasattr(last_message, 'content'):
            ...
            if message_type:
                ...
                # Set the flag and return the message
                return {
                    "messages": [AIMessage(content=fast_response)],
                    "fast_path_taken": True # Flag ensures summarization step is skipped
                }
        ...
        # --- Main LLM Processing Path: Construct Final Prompt with History ---
        messages_to_send = [] 
        
        query_type = state.get("query_type", "unknown")
        enhanced_system_msg = self.system_message.content + f"\n\nQUERY TYPE: {query_type.upper()}"
        messages_to_send.append(SystemMessage(content=enhanced_system_msg))
        
        if chat_history:
            # Include the current summary in the prompt
            memory_context_msg = SystemMessage(
                content=f"PREVIOUS CONVERSATION SUMMARY/HISTORY:\n{chat_history}\n\n"
            )
            messages_to_send.append(memory_context_msg)
            
        messages_to_send.extend(messages)
        # ------------------------------------------------------------------

        try:
            response = self._invoke_llm_with_retry(messages_to_send)
    ...
    # --- STM ADDED: Summarizer Node ---
    def _summarize_chat_node(self, state: State) -> Dict[str, Any]:
        """Node to update the chat_history field in the state using the MemoryManager."""
        
        # Skip summarization if fast path was taken (e.g., simple greeting)
        if state.get("fast_path_taken", False):
            print("[MEMORY] Skipping summarization (Fast Path taken).")
            # Return current history to maintain state structure
            return {"chat_history": state.get("chat_history", "")}

        new_summary = self.memory_manager.update_summary(state)
        return {"chat_history": new_summary}
    # ------------------------------------

    # --- CRITICAL FIX: Custom Chatbot Router to handle tool calls and summarization ---
    def _route_chatbot(self, state: State) -> str:
        """Determines if the chatbot needs to call a tool, or route to summarizer/END."""
        
        messages = state["messages"]
        last_message = messages[-1] if messages else None
        
        # 1. Check for tool calls (routes to 'tools' node)
        if last_message and getattr(last_message, "tool_calls", None):
            return "tools"
        
        # 2. ALL other paths (final answer, fast path, error message) go to summarizer.
        # This ensures a clean, single-point termination via 'summarizer' -> END.
        return "summarizer"
    # ---------------------------------------------------------------------------
    
    def build(self):
        """Build the CAG graph with smart routing and memory summarization"""
        ...
        graph_builder.add_node("summarizer", self._summarize_chat_node)
        
        # Build graph flow
        graph_builder.add_edge(START, "classifier")
        graph_builder.add_edge("classifier", "chatbot")
        
        # Conditional edges from CHATBOT (Now only routes to 'tools' or 'summarizer')
        graph_builder.add_conditional_edges(
            "chatbot",
            self._route_chatbot,
            {
                "tools": "tools",
                "summarizer": "summarizer", # All final answers go here (including fast path)
                # CRITICAL: Removed the END: END transition
            }
        )
        
        graph_builder.add_edge("tools", "grader")
        
        # Conditional edge from GRADER (Synthesis Fix)
        graph_builder.add_conditional_edges(
            "grader",
            self._should_correct,
            {
                "correct": "corrector",
                # CRITICAL FIX: After a successful tool call/grader, route back to 'chatbot' 
                # for final answer synthesis (LLM reads tool output), which then goes to 'summarizer'.
                "continue": "chatbot" 
            }
        )
        graph_builder.add_edge("corrector", "chatbot") # Back to chatbot for correction
        
        # FINISH: Summarizer always leads to END
