...
class GraphBuilder:
    def __init__(self):
        ...
        summary_prompt = PromptTemplate.from_template(
            ...
            Keep the summary concise, under 100 words, and only capture ALL factual user-stated preferences (like their name: **Crucially, if the user states their name, such as 'My name is Anirudh', ensure it is captured in the summary**), preferred stocks, and financial goals.
        ...
        self.summary_chain = LLMChain(llm=self.llm, prompt=summary_prompt) # Use your existing self.llm
...
Choose the RIGHT tool based on the query type."""
    ...
    """def _chatbot_node(self, state: State):
        ...
        if last_message and hasattr(last_message, 'content'):
            ...
            if message_type:
                ...
                print(fast_response)
                return {"messages": [last_message, AIMessage(content=fast_response)]}
        ...
        # [NEW] Retrieve the current summary
        current_summary = state.get("summary", "No prior conversation history.")
        
        # [MODIFIED] Enhance the system message with the summary
        enhanced_system_msg = (
    ...
    def _chatbot_node(self, state: State):
        ...
        message_content = last_message.content
        query_type = state.get("query_type", "unknown") # Get the classification result

        # 1. CACHE HIT CHECK (Highest Priority)
        cached_response = self.response_cache.get(messages)
        if cached_response:
            print("[CACHE HIT] Returning cached response")
            # NOTE: Cached response is already an AIMessage
            return {"messages": [cached_response]} 
            
        # 2. FAST PATH CHECK (Only if the classifier didn't find a complex intent)
        
        # CRITICAL FIX: If the classifier identified a complex intent, skip the Fast Path.
        is_complex_query = query_type in ['stock', 'general', 'knowledge']

        if not is_complex_query:
            # Only run the Fast Path check if the query is NOT complex (truly simple/generic)
            message_type = self.fast_path_router.is_generic_message(message_content)
            
            if message_type:
                # Execute hardcoded response path
                print(message_type)
                fast_response_content = self.fast_path_router.get_fast_response(message_type) 
                
                # NOTE: Returning only the AIMessage is the cleanest way in LangGraph
                return {"messages": [AIMessage(content=fast_response_content)]}


        # 3. FULL LLM EXECUTION (If not cached AND not handled by Fast Path)
        
        # [Summarization Integration] Retrieve the current summary
        current_summary = state.get("summary", "No prior conversation history.")
        print(f"\n[DEBUG] Summary loaded for Chatbot: {current_summary}")
        # Enhance the system message with the summary
        enhanced_system_msg = (
            self.system_message.content + 
            f"\n\n--- CONVERSATION SUMMARY ---(Use this to recall user facts/name, e.g., 'sam')\n{current_summary}" +
            f"\n\nQUERY TYPE: {query_type.upper()}"
        )
        ...
        recent_messages = conversational_messages[-10:]
        messages_with_system = [SystemMessage(content=enhanced_system_msg)] +recent_messages# messages
    ...
    # In GraphBuilder (Add these new methods)

    def _router_to_tools_or_summarizer(self, state: State) -> str:
        """Route from chatbot to tools or summarizer (replaces tools_condition if no tool call)."""
        last_message = state["messages"][-1]
        print(last_message)
        print(hasattr(last_message, "tool_calls"))
        # If the LLM requested a tool call, route to 'tools'
        # This logic is what used to be in tools_condition, but now we divert on END
        if hasattr(last_message, "tool_calls") and last_message.tool_calls:
            return "tools"
        
        # Otherwise, the LLM has generated a final answer, so route to 'summarizer'
        return "summarizer"

    """def _summarization_node(self, state: State):
        ""Node to update the running conversation summary.""
        messages = state["messages"]
        print("line 363->",messages)
        # Use .get() to safely retrieve the summary field added to the State
        current_summary = state.get("summary", "The conversation has just begun.") 
        print("line 366->",current_summary)
        # Take the last 5 relevant messages for summarization.
        conversational_messages = [
            m for m in messages 
            if isinstance(m, (HumanMessage, AIMessage)) and not m.tool_calls
        ]
        
        # Only summarize the last few turns to manage cost/latency
        new_messages = conversational_messages[-5:]
        
        if not new_messages:
            return state

        # Format messages for the summarization chain
    ...
    # In workflow.py, inside GraphBuilder.build(self):
    def _summarization_node(self, state: State):
        """Node to update the running conversation summary."""
        messages = state["messages"]
        print("line 363->",messages)
        ...
        # CRITICAL FIX: Implement defensive coding to prevent AttributeError on HumanMessage.
        # Logic: 
        # 1. Must be a conversational message (Human or AI).
        # 2. AND (either it has no 'tool_calls' attribute OR the attribute is empty).
        conversational_messages = [
            m for m in messages 
        ...
        # Only summarize the last few turns to manage cost/latency
        new_messages = conversational_messages[-8:]#[-5:]
    ...
    def build(self):
        ...
        # 🚨 FINAL CRITICAL FIX: Replace tools_condition with the custom router
        # This custom router routes successful answers to the summarizer
